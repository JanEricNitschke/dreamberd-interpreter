...
# :D
Namespace: TypeAlias = dict[str, Variable | Name]
CodeStatementWithExpression: TypeAlias = (
    ReturnStatement
    | Conditional
    | ExpressionStatement
    | WhenStatement
...
AsyncStatements: TypeAlias = list[
    tuple[list[tuple[CodeStatement, ...]], list[Namespace], int, Literal[1, -1]]
]
NameWatchers: TypeAlias = dict[
    ...
    tuple[
        ...
        set[tuple[str, int]],
        list[Namespace],
        DreamberdPromise | None,
    ],
]
WhenStatementWatchers: TypeAlias = list[
    dict[str | int, list[tuple[ExpressionTreeNode, list[tuple[CodeStatement, ...]]]]]
]  # bro there are six square brackets...


def get_built_expression(expr: list[Token] | ExpressionTreeNode) -> ExpressionTreeNode:
    return (
        expr
...
def evaluate_normal_function(
    expr: FunctionNode,
    func: DreamberdFunction | BuiltinFunction,
...
def declare_new_variable(
    ...
    if lifetime == "Infinity":
        ...
        def remove_lifetime(
            ...
            if lifetime[-1] not in ["s", "m"] or not all(
                c.isdigit() for c in lifetime[:-1]
            ):
                raise_error_at_line(
                    filename,
                    code,
                    error_line,
                    "Invalid time unit for variable lifetime.",
                )
            sleep(
                int(lifetime[:-1]) if lifetime[-1] == "s" else int(lifetime[:-1] * 60)
            )
        ...
        Thread(
            ...
            args=(lifetime, target_var, target_lifetime, current_line),
        ).start()
...
def get_name_and_namespace_from_namespaces(
    name: str, namespaces: list[Namespace]
) -> tuple[Variable | Name | None, Namespace | None]:
...
def is_really_equal(left: DreamberdValue, right: DreamberdValue) -> DreamberdBoolean:
    ...
    match (
        left,
        right,
    ):  # i know these are horribly verbose but if i don't do this my LSP yells at me
        case (
...
def print_expression_debug(
    debug: int,
    expr: list[Token] | ExpressionTreeNode,
    value: DreamberdValue,
    namespaces: list[Namespace],
) -> None:
...
def evaluate_expression_for_real(
    ...
    match expr:
        ...
        case ExpressionNode():  # done :)
            ...
            if (
                db_to_boolean(left).value is True
                and expr.operator == OperatorType.OR
                or db_to_boolean(left).value is False
                and expr.operator == OperatorType.AND
...
def adjust_for_normal_nexts(
    ...
    for (name, ns_id), old_len in zip(normal_nexts, old_normal_vals, strict=False):
...
def register_when_statement(
    condition: list[Token] | ExpressionTreeNode,
    statements_inside_scope: list[tuple[CodeStatement, ...]],
    namespaces: list[Namespace],
    async_statements: AsyncStatements,
    when_statement_watchers: WhenStatementWatchers,
...
def interpret_statement(
    ...
    if all_normal_nexts:
        match (
            statement
        ):  # this is here to make sure everything is going somewhat smoothly
            case (
                VariableAssignment()
                | VariableDeclaration()
                | Conditional()
        ...
        return None  # WE ARE EXITING HERE!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        case ClassDeclaration():
            ...
            def class_object_closure(*args: DreamberdValue) -> DreamberdObject:
                ...
                if constructor := class_namespace.get(class_name.value):
                    ...
                    new_namespace: Namespace = {
                        name: Name(name, arg)
                        for name, arg in zip(func.args, args, strict=False)
                    }
                    interpret_code_statements(
                        func.code,
...
def interpret_code_statements(
    ...
    while 0 <= curr < len(statements):
        ...
        if isinstance(
            ...
            return retval
        if isinstance(statement, ReverseStatement):
            direction = -direction
