...
def get_expr_first_token(expr: ExpressionTreeNode) -> Token | None:
    match expr:
                        ...
                        return ListNode(
                            [
                                build_expression_tree(
                                    filename,
                                    tokens[
                                        int(starts_with_whitespace) + 1 : len(tokens)
                                        - int(ends_with_whitespace)
                                        - 1
                                    ],
                                    code,
    ...
    if updated_list[max_index] == OperatorType.COM:
        ...
        if tokens_without_whitespace[
            ...
            raise_error_at_token(
                filename,
                code,
                "Expected function call. This is likely an issue of whitespace, as DreamBerd replaces parentheses with spaces and has significant whitespace.",
                tokens_without_whitespace[0],
            )
